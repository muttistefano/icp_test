...
# x_labels = x_labels/0.02
# y_labels = y_labels/0.02
# w_labels = w_labels/0.02
...
with no_grad():
    for cnt,data in enumerate(ranges_data_array):
        inputs = from_numpy(data[0:3].astype(np.float32)).to(device)[None, :]
        outputs = model(inputs)
        # print(w_labels[cnt],outputs.cpu().numpy()[0][0])
        outsx.append((outputs[0].cpu().numpy()[0][0]*x_labels.std()) + x_labels.mean() )
        outsy.append((outputs[1].cpu().numpy()[0][0]*y_labels.std()) + y_labels.mean())
        outsz.append((outputs[2].cpu().numpy()[0][0]*w_labels.std()) + w_labels.mean())


plt.figure("x")
plt.plot(range(len(x_labels)),x_labels,label="labels")
plt.plot(range(len(x_labels)),outsx,label="out")
plt.legend()

plt.figure("y")
plt.plot(range(len(x_labels)),y_labels,label="labels")
plt.plot(range(len(x_labels)),outsy,label="out")

plt.legend()
plt.figure("w")
plt.plot(range(len(x_labels)),w_labels,label="labels")
plt.plot(range(len(x_labels)),outsz,label="out")
...
#TESTA SU BLANKER, SALVA I PARAMETRI DELLO SCALING 
